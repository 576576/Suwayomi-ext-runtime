...
> 状态：**剥离进行中**。当前仓库只有骨架（构建配置 / 文档 / 发布流水线），模块目录 `ext-runtime/` 由剥离流程 P2 导入。
> 完整施工计划见 [`docs/EXTRACTION_PLAN.md`](docs/EXTRACTION_PLAN.md)。
...
一个坐标、两个制品（同一 package，授权只需一次）：

| 制品 | 用途 |
|---|---|
| `ext-runtime-<version>.jar` | **桌面**：fat jar，部署为 `<发布根>/bin/ext-runtime.jar`，由 Suwayomi-next 的 server 拉起 |
| `ext-runtime-<version>-shared-sources.jar`（classifier `shared-sources`） | **Android**：`Suwayomi-next/android/extension-host` 解压成目录后作为额外源根编译 |

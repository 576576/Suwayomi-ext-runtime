#!/usr/bin/env bash
# 用 jlink 生成**裁剪后的** Java 运行时，作为 `ext-runtime-jre-<V>-<os>-<arch>` 发布资产。
#
# 用法：
#   scripts/make-jre.sh <windows|linux|mac> <x64|aarch64> <输出目录>
#
# 例：scripts/make-jre.sh windows x64 out/jre
#
# **为什么这个脚本在本仓库而不是 Suwayomi-next**：这份 JRE 存在的唯一目的是跑
# `ext-runtime.jar`（服务端是 Rust 二进制，托盘是原生可执行文件，都不需要 JVM）。
# 所以模块白名单完全由沙盒的需求决定 —— `jdk.httpserver` 是沙盒自己的 HTTP 宿主、
# `java.prefs` 是共享源码里 PersistentCookieStore 用的、`--include-locales=en,ja,zh`
# 是为扩展站的日/中文站点。白名单与沙盒代码必须同仓演进，否则改沙盒加了个模块，
...
# >>> binary-probe >>>
# 下面这组函数被 .workbuddy-ai/verify/check_jre_arch.sh 按标记**整块抽出去**单测，
...
#   java.prefs         —— ext-runtime 共享源码里的 PersistentCookieStore 用 java.util.prefs。
